...
def single_epoch_training(model: VENL, loader: DataLoader, optimizer: optim):
    ...
    for batch_idx, (images, controls) in enumerate(trainBar):
        ...
        raw_weight_ent = (weights.squeeze() * torch.log(weights.squeeze() + 1e-9))
        if len(raw_weight_ent.shape) == 2:
            entropy_w = -raw_weight_ent.sum(dim=1).mean()
        else:
            entropy_w = -raw_weight_ent.sum(dim=0)
        ...
        grad_cam0 = grad_mean_abs(model.multicam_backbone[0])
        grad_cam1 = grad_mean_abs(model.multicam_backbone[1])
        grad_cam2 = grad_mean_abs(model.multicam_backbone[2])
        ...
        trainMetrics["Grad_MultiCam0"]  += grad_cam0
        trainMetrics["Grad_MultiCam1"]  += grad_cam1
        trainMetrics["Grad_MultiCam2"]  += grad_cam2
        ...
        avg_grad_cam      = np.mean([trainMetrics["Grad_MultiCam0"], 
                                     trainMetrics["Grad_MultiCam1"], 
                                     trainMetrics["Grad_MultiCam2"]]) / step
