...
class CarlaDatasetLoader:
    def __init__(self, dataset_dir: str | list[str], images_key: list[str] = None, downsize_ratio = 1, input_metadata = None, load_size: int = -1, shuffle = True):
        self.log = Logger()
    ...
    def read_image(self, img_path: Path, key_name):
        ...
        if self.input_metadata is not None:
            image = cv2.resize(image, self.input_metadata[key_name][2:][::-1])
    ...
    def _get_samples(self, idx):
        ...
        if self.images_key is None:
            ...
            for key_name in self.images_key:
                ...
                image = self.read_image(str(img_file), key_name)
