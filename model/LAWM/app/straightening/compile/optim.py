...
def compile_opt(
    ...
    param_groups = [
        {
            "params": (p for n, p in filterer.named_parameters() if ("bias" not in n) and (len(p.shape) != 1)),
        ...
        {
            "params": (p for n, p in filterer.named_parameters() if ("bias" in n) or (len(p.shape) == 1)),
