...
def main(args: dict, yaml_path: str):
    ...
    def eval_step(clips):
        ...
        def forward_prediction(h_ctx: torch.Tensor, h_goal: torch.Tensor, T: int, H: int = None, W: int = None):
            ...
            # -- Autoregressive rollout from independently encoded frame 0
            h_seed = forward_context(to_latent(clips[:, :, :1]), H, W)
            z_ctx = h_seed
        ...
        with torch.amp.autocast(device_type, dtype = dtype, enabled = mixed_precision):
            ...
            z_tf, z_ar, a_tf = forward_prediction(h_ctx, h_goal, T, H_patches, W_patches)
        ...
        print(f"Teacher Forcing: {loss_tf} | Autoregressive Rollout: {loss_ar}")
    ...
    for sample in video_loader:
        
        clips = sample.to(device)
        
        (loss, loss_tf, loss_ar, loss_act, loss_collapse, loss_straight, energy, vcm), elapsed_time = gpu_timer(partial(eval_step, clips))
