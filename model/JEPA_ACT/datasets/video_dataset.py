...
from model.JEPA_ACT.datasets.utils.dataloader import ConcatIndices, MonitoredDataset, NondeterministicDataLoader
from model.JEPA_ACT.datasets.utils.weighted_sampler import DistributedWeightedSampler
...
class VideoDataset(torch.utils.data.Dataset):
    """Video / Image classification dataset."""
    ...
    def __init__(
        ...
        # -- This is to choose sampling strategy
        ...
        # -- Get only how many frames per clip (default is 16)
        ...
        # -- The reverse of np.cumsum, where it gives the local index of a dataset given global index (it merges 2 different datasets together)
    ...
    def get_item_video(self, index):
        ...
        # -- Transforms entire buffer (video)
        if self.shared_transform is not None:
            buffer = self.shared_transform(buffer)
        buffer = split_into_clips(buffer)
        # -- Individual transforms per clip
    ...
    def loadvideo_decord(self, sample, fpc):
        ...
        Process for one video, partition it out into multiple clips
        and return the list of clips.
        """
        ...
        for i in range(self.num_clips):
            # -- can jiggle the video inside the partition given that the calculated length is smaller then the partition size
            if partition_len > clip_len:
                ...
                # then start_indx of segment i+1 will lie within segment i
                    ...
                    # -- Override the partition with a clip step
                    clip_step = 0
                    if len(vr) > clip_len:
