...
class VisionTransformerPredictor(nn.Module):
    ...
    def forward(self, x, masks_x, masks_y, mask_index=1, has_cls=False):
        """
        :param x: context tokens [B, N_ctxt, D]
        :param masks_x: indices of context tokens in input [B, N_ctxt]
        :param masks_y: indices of target tokens in input [B, N_pred]
        """
        ...
        # Make target tokens
        # -- mask_index is the sub batch index
        # -- dictates what learnable weights is suitable for which scale, the data has consistent order to sub_batch (fpc ordering)
        mask_index = mask_index % self.num_mask_tokens
        pred_tokens = self.mask_tokens[mask_index]
        pred_tokens = pred_tokens.repeat(B, self.num_patches, 1)
        pred_tokens = apply_masks(pred_tokens, masks_y) # -- (B, num_pred, D)
        # -- add pos embed
        if not self.use_rope:
        ...
        # -- X should be the same because of the wrapper
        ...
        # -- This should be the full tokens if we used complements
        # -- Sort all these tensors back in order in case we use RoPE
