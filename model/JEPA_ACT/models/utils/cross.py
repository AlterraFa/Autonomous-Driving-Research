...
class CrossRoPEAttention3D(nn.Module):
    def __init__(
        self, 
        dim,
        num_heads=8,
    ...
    def forward(self, x1, x2, attn_mask = None, T = None, H = None, W = None, action_pframe = 1):
        # -- No need for mask as CrossRoPE is not used for pretraining
        
        B2, N2, C2 = x2.shape
        # -- (2, B, num_heads, tokens, dim_per_head)
        kv: torch.Tensor = self.kv(x2).reshape(B2, N2, 2, self.num_heads, C2 // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        
        B1, N1, C1 = x1.shape
        grid_depth = int(N1 // (self.grid_size * self.grid_size))
        q: torch.Tensor = self.q(x1).reshape(B1, N1, self.num_heads, C1 // self.num_heads).permute(0, 2, 1, 3)

        if T is None or H is None or W is None:
            mask = torch.arange(int(grid_depth * self.grid_size * self.grid_size), device = q.device)
        else:
            mask = torch.arange(int(T * H * W), device = q.device)
        d_mask, h_mask, w_mask = self.separate_position(mask, H, W)
        
        
        s = 0
        qd = rotate_queries_or_keys(q[..., s: s + self.d_dim], pos = d_mask); s += self.d_dim
        qh = rotate_queries_or_keys(q[..., s: s + self.h_dim], pos = h_mask); s += self.h_dim
        qw = rotate_queries_or_keys(q[..., s: s + self.w_dim], pos = w_mask); s += self.w_dim
        ...
        num_action = N2 // action_pframe
        action_mask = torch.arange(num_action, device=k.device).repeat(action_pframe)
        
        k = k.view(B2, self.num_heads, action_pframe, num_action, -1)
        k_rotated = []
        
        for idx in range(action_pframe):
            ki = k[:, :, idx, :, :]  # (B, heads, num_action, dim)
            ki_d = rotate_queries_or_keys(ki[..., :self.d_dim], pos=action_mask[:num_action])
            if ki.shape[-1] > self.d_dim:
                ki_r = ki[..., self.d_dim:]
                ki_rotated = torch.cat([ki_d, ki_r], dim=-1)
            else:
                ki_rotated = ki_d
            k_rotated.append(ki_rotated)
        
        k = torch.stack(k_rotated, dim=2).view(B2, self.num_heads, N2, -1)
...
if __name__ == "__main__":
    ...
    dummy1 = torch.randn((3, 1568, 512))
    dummy2 = torch.randn((3, 12 * action_pframe, 512))
    
    attn(dummy1, dummy2, T = 8, H = 14, W = 14, action_pframe = action_pframe)