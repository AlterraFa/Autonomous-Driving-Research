...
from model.JEPA_ACT.models.utils.patch_embed import PatchEmbed, PatchEmbed3D
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        if not self.use_rope:
            # -- Patch embed is a fixed value tensor -> interpolation is needed
        ...
        if masks is not None:
            # -- [(B, indices), ...] * num_cfg
            # -- In actuality, there's only [(B, indices)] since there's a wrapper that iterates through the masks
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_activation_checkpointing:
                # -- disable saving computed tensors and recompute them during backward pass via autograd
